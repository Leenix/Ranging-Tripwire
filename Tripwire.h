...
typedef long (*range_function)(
    void); /**< Passed function to get the range of the sensor */
typedef void (*event_callback)(
    void); /**< Callback function for sensor events */
...
public:
  ...
  */
  void start(long distance_threshold);
  ...
  */
  ...
  unsigned long
      event_start_time; /**< Time that the last sensor event started [ms] */
...
private:
  ...
  const static long DEFAULT_DISTANCE_THRESHOLD =
      70; /**< Default threshold distance [implied cm] */
  const static int DEFAULT_MIN_BASELINE_READS =
      20; /**< Default minimum number of baseline reads for calibration */
  const static int DEFAULT_MAX_BASELINE_READS =
      40; /**< Default maximum number of baseline reads for calibration */
  ...
  const static int DEFAULT_MINIMUM_SUCCESSIVE_DETECTIONS =
      0; /**< Default number of successive sensor detections */
}
