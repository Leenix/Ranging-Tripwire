...
Tripwire::Tripwire(range_function get_range) {
  ...
  */
  ...
  max_baseline_read = DEFAULT_MAX_BASELINE_READS;
...
Tripwire::start(unsigned long threshold) {
  /**
  * Start the tripwire.
...
void Tripwire::calibrate() {
  ...
  */
  ...
  if (_get_range != NULL && _distance_threshold != NULL) {
    ...
    while (
        (baseline_reads < min_baseline_reads || average_variance > variance) &&
        baseline_reads < MAX_BASELINE_READS) {
      long new_range = (*_get_range)();
      long new_variance = abs(average - new_range);

      average_variance = ((average_variance + new_variance) / 2);
      baseline_distance = ((average_range + new_range) / 2);
